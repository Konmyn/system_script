...
# postgres安装的执行文件路径，最后加入系统path当中
...
def pgdbBackup(pwd, db_list, file_path, file_prefix):
    ...
    for datebase in db_list:
        ...
        if not os.system(backup_cmd):
            writeLogs(log_path, "Backup for *{}* completed.\n".format(datebase))
        else:
...
def fileBackup(directory, file_path, file_prefix):
    ...
    if not os.system(zip_cmd):
        writeLogs(log_path, "Backup for #filestore# completed.\n")
        return True
    else:
...
def backupDelete(file_path, term=7):
    ...
    for _file in _files:
        if _file[:len(prefix)] == prefix:
            ...
            try:
                os.remove(file_path)
                writeLogs(log_path, "Delete completed for file: {}\n".format(_file))
            except:
                writeLogs(log_path, "Delete failed for file: {}\n".format(_file))
    return True
...
def main():
    ...
    '''
    writeLogs(log_path, "Operation time: {}\n".format(today_str))
